...
def search(end_date: datetime.date, translate: bool):
    finished = False
    article_objects = []
    page_number = 0
        ...
        for a in articles:
            ...
            if date != '':
                ...
                if category not in ['Sports', 'Diaspora Covid-19']:
                    article_objects.append(Article(headline, url, date, category, text, translate=translate))
                if date.date() < end_date:
                    finished = True
                    break
            else:
                date = article_objects[-1].date
                article_objects.append(Article(headline, url, date, text, translate=translate))
